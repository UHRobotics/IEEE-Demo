/* Automerge of "Demo_linefollowing.c", "Demo_linefollowing - Copy.c", and
 * "Demo_linefollowing - for An.c" by git-merge-file, 20150621
 * 
 * Merge format:
 * 
 * A conflict occurs if both <current-file> and <other-file> have changes in a common segment of
 * lines. If a conflict is found, git merge-file normally outputs a warning and brackets the
 * conflict with lines containing <<<<<<< and >>>>>>> markers. A typical conflict will look like
 * this:
 * 
 * <<<<<<< A
 * lines in file A
 * =======
 * lines in file B
 * >>>>>>> B
 * 
 */
#pragma config(Sensor,    in1,      rlight,        sensorReflection                )
#pragma config(Sensor,    in2,      midlight,      sensorReflection                )
#pragma config(Sensor,    in3,      llight,        sensorReflection                )
#pragma config(Sensor,    dgtl1,    ultrasonic,    sensorSONAR_mm                  )
#pragma config(Motor,     port1,    rfront,        tmotorVex269,       openLoop    )
#pragma config(Motor,     port2,    rback,         tmotorVex269,       openLoop    )
#pragma config(Motor,     port3,    lfront,        tmotorVex269,       openLoop    )
#pragma config(Motor,     port4,    lback,         tmotorVex269,       openLoop    )
#pragma config(Motor,     port5,    rollers,       tmotorVex269,       openLoop    )
#pragma config(Motor,     port6,    rarmbot,       tmotorVex269,       openLoop    )
#pragma config(Motor,     port7,    rarmtop,       tmotorVex269,       openLoop    )
#pragma config(Motor,     port8,    larmbot,       tmotorVex269,       openLoop    )
#pragma config(Motor,     port9,    larmtop,       tmotorVex269,       openLoop    )
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

//void setup()
//{
//  setBaudRate(uartSystemCommPort,baudRate115200);
//}
//void send_val(int val)
//{
//  sendChar(uartSystemCommPort,(short)(val));
//}
task main()
{
  // int low1;
  // int low2;
  // int high;
  // int threshold;

  // while(true)
  // {
    // /*Wait for 1/2 sec then go full speed with rollers for 3 seconds, turn 180, go full speed for
     // *3 seconds, turn 180 again, return to code
     // */
    // if(SensorValue(ultrasonic)</*insert value here*/)
    // {
      // wait1Msec(500)
      // motor[rfront]=120;
      // motor[rback]=120;
      // motor[lfront]=-120;
      // motor[rfront]=-120;
     // motor[rollers]=120;
      // wait1Msec(3000)

      // wait1Msec(500)

      // motor[rfront]=-120;
      // motor[rback]=-120;
      // motor[lfront]=120;
      // motor[rfront]=120;
      // wait1Msec(3000)

     // wait1Msec(500)
    // }
    // else
    // {
      // /*insert line following code here*/
    // }

    /*motor[rfront]=vexRT[Ch2];   //driver control
    motor[rback]=vexRT[Ch2];
    motor[lfront]=-vexRT[Ch3];
    motor[lback]=-vexRT[Ch3];

    if(vexRT[Btn5U]==1)
    {
      motor[rarmtop]=120;
      motor[rarmbot]=120;
      motor[larmtop]=-120;
      motor[larmbot]=-120;
    }
    else if(vexRT[Btn5D]==1)
    {
      motor[rarmtop]=-120;
      motor[rarmbot]=-120;
      motor[larmtop]=120;
      motor[larmbot]=120;
    }
    else
    {
      motor[rarmtop]=0;
      motor[rarmbot]=0;
      motor[larmtop]=0;
      motor[larmbot]=0;
    }
    if(vexRT[Btn6U]==1)
    {
      motor[rollers]=120;
    }
    else if(vexRT[Btn6D])
    {
      motor[rollers]=-120;
    }
    else
    {
      motor[rollers]=0;
    }

    if(vexRT[Btn8U]==1)
    {
      applesauce=1;
      //low1=SensorValue(rlight);
      //low2=SensorValue(llight);
      //high=SensorValue(midlight);
      //threshold=(((low1+low2)/2)+high)/2;
    }


      int whiteLine;*/


      /*Testing lightsensors*/
      /*Start*/

        int whiteLine=SensorValue(midlight);




      //if((SensorValue(midlight) <= (whiteLine+10)) || (SensorValue(midlight) >= (whiteLine-10)) )
      //{
      //motor[rfront]=50;
      //}
      /*End*/




  //  applesauce = 1;


      while(true)   //line following
      {

        if(SensorValue(rlight) <= (whiteLine + 500))  //If the right sensor detects white go right.
        {
          motor[port1] = 25;
          motor[port2] = 25;
          motor[port3] = -50;
          motor[port4] = -50;
          wait1Msec(500);
        }
        else if(SensorValue(llight) <= (whiteLine + 500))  //If the left sensor detects white go left.
        {
          motor[port1] = 50;
          motor[port2] = 50;
          motor[port3] = -25;
          motor[port4] = -25;
          wait1Msec(500);
        }
        else if(SensorValue(midlight) <= (whiteLine+500))  // If all sensors read white go foward
        {
          motor[port1] = 50;
          motor[port2] = 50;
          motor[port3] = -50;
          motor[port4] = -50;
          wait1Msec(500);
        }
        else
        {
          motor[port1]=0;
          motor[port2]=0;
          motor[port3]=0;
          motor[port4]=0;
        }
<<<<<<< Demo_linefollowing - Copy.c
          motor[port1]=0;
          motor[port2]=0;
          motor[port3]=0;
          motor[port4]=0;

=======

        motor[port1]=0;
        motor[port2]=0;
        motor[port3]=0;
        motor[port4]=0;

        if(vexRT[Btn8D]==1)
        {
          applesauce=0;
        }
>>>>>>> Demo_linefollowing.c
      }
}